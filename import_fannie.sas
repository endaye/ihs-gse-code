...
%put ~~~~acquisition&period.~~~~performance&period.;
  ...
  data id_loan_pf;
    set performance_&period.;
    keep id_loan Current_Int_rt dt_matr msa servicer_name period delq_sts mths_remng CD_Zero_BAL Dt_zero_BAL;
  run;
  proc sort; by id_loan; run;
  ...
  data loan_float_p;
    set loan;
    if strip(delq_sts) ^= '0' or Dt_zero_BAL ^= .;
    if strip(delq_sts) = 'X' or strip(delq_sts) = 'x' then delq_sts = '10';
  run;
  ...
  proc transpose data=loan_float_p(keep=id_loan delq_sts period) out=dt_delq prefix =dt_delq_;
    by id_loan;
    id delq_sts;
    var period;
  run;

  proc transpose data=loan_float_p(keep=id_loan delq_sts mths_remng) out=mths_remng_delq prefix =mths_remng_delq_;
    by id_loan;
    id delq_sts;
    var mths_remng;
  run;
